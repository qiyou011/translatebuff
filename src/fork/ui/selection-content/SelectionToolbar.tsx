import type { ModalDialogHostController } from "@/entrypoints/selection.content/selection-toolbar/modal-dialog-host"
...
import {
  CloseButton,
  DropEvent,
} from "@/entrypoints/selection.content/selection-toolbar/close-button"
import { createModalDialogHostController } from "@/entrypoints/selection.content/selection-toolbar/modal-dialog-host"
import {
...
} from "@/entrypoints/selection.content/selection-toolbar/positioning"
import { SpeakButton } from "@/entrypoints/selection.content/selection-toolbar/speak-button"
import { buildContextSnapshot, readSelectionSnapshot } from "@/entrypoints/selection.content/utils"
...
// 与其 module-private helpers 逐段照搬、只把相对 import 改成绝对路径），不重写机制；仅重写药丸 JSX
// 内层——渲染 fork TranslateButton / fork CustomActionTrigger（消费 fork context，见 D4 三元组绑定），
// 配 fork context 之外的上游叶子 SpeakButton / CloseButton。
//
// D4 三元组绑定（必守，否则运行时崩）：本 Toolbar 承载的 fork 触发按钮读 fork 自建 context，绝不 import
// 上游 translate-button/provider 或 custom-action-button/provider（那两处 context 是 module-private，
// 混用会抛 "must be used within Provider"）。
//
// 同步注意：上游若改 selection-toolbar/index.tsx 的检测/定位机制，需把 delta 搬进本文件。
