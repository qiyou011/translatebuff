...
import FloatingButton from "@/entrypoints/side.content/components/floating-button"
import { configFieldsAtomMap } from "@/utils/atoms/config"
import { sendMessage } from "@/utils/message"
import ForkFloatingButton from ".."
...
describe("fork floatingButton controls", () => {
  it("cancels an active drag on fullscreen and restores the widget on exit", async () => {
    vi.useFakeTimers()
    Object.defineProperty(document, "fullscreenElement", { configurable: true, value: null })
    renderFloatingButton()
    fireEvent.pointerDown(getMainButton(), {
      pointerId: 1,
      pointerType: "mouse",
      button: 0,
      clientX: 900,
    ...
    expect(screen.queryByTestId("floating-main-button")).toBeNull()
  ...
  it.each(["Enter", " "])("opens the panel with the %s keyboard shortcut", (key) => {
    renderFloatingButton()
    fireEvent.keyDown(getMainButton(), { key })
