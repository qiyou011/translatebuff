...
describe("providerSpecificSettingsField", () => {
  ...
  it("renders Bedrock settings from Zod metadata and persists changes", async () => {
    ...
    const regionInput = screen.getByLabelText(
      "options.apiProviders.form.providerSettingLabels.region",
    )
  ...
  it("renders Azure settings without a region field and persists non-empty values", async () => {
    ...
    expect(
      screen.queryByLabelText("options.apiProviders.form.providerSettingLabels.region"),
    ).not.toBeInTheDocument()

    const apiModeSelect = screen.getByLabelText(
      "options.apiProviders.form.providerSettingLabels.apiMode",
    )
    const resourceNameInput = screen.getByLabelText(
      "options.apiProviders.form.providerSettingLabels.resourceName",
    )
    const apiVersionInput = screen.getByLabelText(
      "options.apiProviders.form.providerSettingLabels.apiVersion",
    )

    expect(apiModeSelect).toHaveTextContent(
      "options.apiProviders.form.providerSettingOptionLabels.responses",
    )
