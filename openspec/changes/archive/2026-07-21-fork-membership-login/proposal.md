...
- 动态网关与模型：base_url 与 sk_key 同源 `/v1/tokens` 一起下发、动态注入 provider（归一含 `/v1`，设置页「Base URL」只读展示）；登录后主动拉一次 `/v1/models` 重建可用模型集（对齐 aimanager 薄凭据两段式），provider 开箱可翻译。
- 体验收尾：接管时序顺序两段（手机号先写、登录态即时展示，不被 tokens 开户轮询阻塞）；账户菜单手机号脱敏展示；登录引导条仅未登录时显示。
...
- 本次不做：付费/下单、temp_token 反向 SSO（跳官网付费免登）、极验人机校验（留在官网侧）；不新写翻译请求代码（注入 base_url+sk_key+models 后由 read-frog 既有引擎自行翻译）。
