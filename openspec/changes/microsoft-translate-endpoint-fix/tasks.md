...
- [ ] 6.9 经 `/jyopsx-retro` 复盘，把 R1（适配器接管）写进 `FORK.md` 的 take-theirs 热点清单
- [ ] 6.10 **经用户明确同意后**提交并推分支，PR 到 `change/fork-foundation`（非 `main`）；交付说明附命令原文与输出、改动文件清单、已知限制（R2/R4/R5）
